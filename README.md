...
5. 既定の `F9`（設定画面で入力欄を選び、`F18`や`Ctrl+M`へ変更・解除可能）で描画モードを
   切り替える。Windows Pointer API対応の
   ペンでは筆圧をペン・マーカーへ、傾きをマーカーへ反映する。右ボタンを押したまま
   移動すると、内周の描画ツールと
   ...
   Undo、Redo、全消去を操作できる。円の横のレイヤーパネルでは最大8枚のレイヤーを追加・選択・
   内容消去・削除でき、いずれもUndo／Redo可能。メニューを閉じた描画モード中は`Delete`でも
   現在レイヤーの内容だけを確認なしで消去できる。消しゴムと「選択」ツールは現在レイヤーだけを
   対象にする。「選択」ツールでは配置済みの図形／スタンプを選び、枠内で
   移動、四隅で縦横比固定の拡大縮小、上側のハンドルで回転できる。`Escape`で操作を取り消せる。
...
全消去前の確認画面も設定から有効・無効を選べます（既定は有効です）。
...
[CONTRIBUTING.md](CONTRIBUTING.md)を参照してください。現在はSignPath Foundationの承認・設定前の
ため配布バイナリはコード署名していません。適格性、脅威モデル、署名有効化条件、利用者による
確認方法は[Code signing policy](docs/code-signing.md)に記録しています。
